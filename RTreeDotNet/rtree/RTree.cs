    ...
    public class RTree : ISpatialIndex
{
   ...
   */
    ...
    //  nearest(Point, Predicate<int>, float)
    //  intersects(Rectangle, Predicate<int>)
    //  contains(Rectangle, Predicate<int>)
    //  size()
    //-------------------------------------------------------------------------
    /**
   ...
   */
    ...
    /**
   * @see net.sf.jsi.SpatialIndex#nearest(Point, Predicate<int>, float)
   */

    public void Nearest(Point p, Predicate<int> v, float furthestDistance)
    {
        ...
        foreach (var nearestId in nearestIds)
        {
            v(nearestId);
        }
        nearestIds.Clear();
        ...
        /**
   * @see net.sf.jsi.SpatialIndex#nearestNUnsorted(Point, Predicate<int>, int, float)
   */

    public void NearestNUnsorted(Point p, Predicate<int> v, int count, float furthestDistance)
    {
        ...
        {
            v(distanceQueue.getValue());
            distanceQueue.pop();
        }
    }

    /**
   * @see net.sf.jsi.SpatialIndex#nearestN(Point, Predicate<int>, int, float)
   */

    public void NearestN(Point p, Predicate<int> v, int count, float furthestDistance)
    {
        PriorityQueue distanceQueue = new PriorityQueue(PriorityQueue.SORT_ORDER_DESCENDING);
        createNearestNDistanceQueue(p, count, distanceQueue, furthestDistance);
    ...
    /**
   * @see net.sf.jsi.SpatialIndex#intersects(Rectangle, Predicate<int>)
   */

    public void Intersects(Rectangle r, Predicate<int> v)
    {
        Node rootNode = getNode(rootNodeId);
    ...
    /**
   * @see net.sf.jsi.SpatialIndex#contains(Rectangle, Predicate<int>)
   */

    public void Contains(Rectangle r, Predicate<int> v)
    {
   ...
   */
    ...
    private void pickSeeds(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId,
        Node newNode)
    {
        ...
        float maxNormalizedSeparation = -1;
            // initialize to -1 so that even overlapping rectangles will be considered for the seeds
   ...
   */
    ...
    private bool intersects(Rectangle r, Predicate<int> v, Node n)
    {
        for (int i = 0; i < n.entryCount; i++)
        {
   ...
   */
    ...
    internal Node chooseNode(float minX, float minY, float maxX, float maxY, int level)
    {
                ...
                if ((tempEnlargement < leastEnlargement) ||
                    ((tempEnlargement == leastEnlargement) &&
                     (Rectangle.area(tempMinX, tempMinY, tempMaxX, tempMaxY) <
                      Rectangle.area(n.entriesMinX[index], n.entriesMinY[index], n.entriesMaxX[index],
                          n.entriesMaxY[index]))))
                {
                    index = i;
   ...
   */
    ...
    private bool checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR)
    {
        ...
        if (expectedMBR != null && object.ReferenceEquals(actualMBR, expectedMBR))
        {
        ...
        for (int i = 0; i < n.entryCount; i++)
        {
            ...
            if (n.level > 1)
            {
                // if not a leaf
                if (
                    !checkConsistency(n.ids[i], n.level - 1,
